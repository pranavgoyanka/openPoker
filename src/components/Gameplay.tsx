...
const Gameplay = ({ buyin, players, setStartGame }: GameplayProps) => {
  ...
  useEffect(() => {
    ...
    const maxIndex = tempWin.reduce((maxIdx, num, idx, arr) => 
  ...
  return (
    <>
      ...
      <p className='text-right mt-2'>Buy In: {buyin}</p>
      <p className='text-right'>Total Pot: {totalPot}</p>
      <p className='text-right text-fuchsia-300 font-bold'>Leader:   {leader}</p>
